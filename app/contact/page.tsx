...
import { getSiteSection, getAllSiteSettings } from '@/lib/site-content'
    ...
    <div className="min-h-screen flex flex-col" style={{ background: 'var(--background)' }}>
      ...
      <div className="pt-[60px] brand-hero-gradient text-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-20">
          <p className="brand-label">{section.subtitle ?? 'ติดต่อเรา — TRIPIRA'}</p>
          <div className="brand-divider opacity-60" />
          <h1 className="text-3xl sm:text-4xl font-bold mb-3 tracking-[-0.02em] leading-tight">{section.title ?? 'ติดต่อ TRIPIRA'}</h1>
          <p className="text-blue-200/80 text-[15px] max-w-lg leading-relaxed">{section.description ?? 'บริษัท ไตรพีระ จำกัด — พร้อมรับฟังความต้องการด้านวิศวกรรมและภูมิสถาปัตยกรรมทุกรูปแบบ'}</p>
        </div>
      </div>
